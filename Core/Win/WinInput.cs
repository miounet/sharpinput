...
namespace Core.Win
{
    ...
    public class WinInput :System.Windows.Forms.Form,IOut
    {
        ...
        public static bool zzspace=false;
        /// <summary>
        /// 用户释放任意键发生
        /// </summary>
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (Input.IsPressLAlt && InputStatus.inputstr.Length == 4 && !InputStatus.inputstr.StartsWith("'"))
                        {
                            ...
                            {
                                if (tpos == 4)
                                    InputStatus.ShangPing(Input.IsPresAltPos+1, 0, false);
                                else
                        ...
                        else
                        {
                            ...
                            if (srspace)
                                InputStatus.Left += InputStatusFrm.LastSPValue.Length * 16;
                            else if (InputMode.stopup.IndexOf(InputStatus.inputstr.Substring(0, 1)) < 0 &&
                                    InputMode.spaceaout == 3 && InputStatus.inputstr.Length == 4 && InputStatusFrm.cachearry.Length > 1)
                            {
                                if (tpos == 4)
                                    InputStatus.ShangPing(2, 0, false);
                                else
                        ...
                        else if (InputStatus.inputstr.Length == 2 && (Input.IsPresAltPos == 3 ) && InputStatusFrm.cachearry.Length > 1 && InputStatusFrm.cachearry[0].Split('|')[1].Length > 1)
                        {
                            Input.IsPressLAlt = false;
                            Input.IsPressRAlt = false;
            ...
            if (curMouseTrac || curTrac || InputMode.imgsend)
            {
                ...
                if (InputStatus.inputstr == "")
                {
                    ...
                    else
                    {
                        ...
                        if (curPoint.Y > 0)
                        {
                            if (curPoint.Y  != InputStatus.Top)
                                InputStatus.Top = curPoint.Y ;
        ...
        private void Initializenotifyicon()
        {
            ...
            TrayIcon.Text = "速录宝3.1.6";//鼠标移至托盘的提示文本
            TrayIcon.Visible = true;

            //定义一个MenuItem数组，并把此数组同时赋值给ContextMenu对象 
            mnuItms = new MenuItem[14];
            mnuItms[mnuItms.Length - 14] = new MenuItem();
            mnuItms[mnuItms.Length - 14].Text = "关于速录宝3.1.6";
            ...
            itmetools44.Text = "词条编辑";
            itmetools44.Click += new System.EventHandler(this.CreateDictEdit);
            mnuItms[mnuItms.Length - 6].MenuItems.Add(itmetools44);

            MenuItem itmetools441 = new MenuItem();
            itmetools441.Text = "词库管理";
            itmetools441.Click += new System.EventHandler(this.MrgDictEdit);
            mnuItms[mnuItms.Length - 6].MenuItems.Add(itmetools441);
        ...
        private void MrgDictEdit(object sender, System.EventArgs e)
        {
            DictMrg frm = new DictMrg();
            frm.Show();
        }
